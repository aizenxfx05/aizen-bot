...
<h3>Aizen XFX Bot — Python Discord Bot + FastAPI Backend</h3>
...
This folder contains the Aizen XFX Discord bot built on `discord.py v2` alongside a `FastAPI` backend that powers the web dashboard. Everything runs from a single `python CodeX.py` command.
...
│   └── aizen_xfx/             Core Aizen XFX feature cogs
...
- Aizen XFX watermark on every source file
...
2. Choose **Cloudflared**, name it (e.g. `aizen-xfx-api`), save
