...
from core import Cog, AizenBot, Context
...
class Music(commands.Cog):
    ...
    async def play_source(self, ctx, query):
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(view=CV2(f"{ZPLUS} Added playlist [{tracks.name}](https://discord.gg/M8qJ9W7vBb) with **{len(tracks.tracks)} songs** to the queue."))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(view=CV2(f"{ZPLUS}   Added [{track.title}](https://discord.gg/M8qJ9W7vBb) to the queue."))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(view=CV2(f"{ZPLUS} Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/M8qJ9W7vBb)** to the queue."))
                await lmao.delete()
                ...
                await ctx.send(view=CV2(f"{ZPLUS} Added all tracks from album **[{album_info['name']}](https://discord.gg/M8qJ9W7vBb)** to the queue."))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def nowplaying(self, ctx: commands.Context):
        ...
        view = CV2Embed(
            title="🎶 Now Playing",
            color=0x1DB954 if source_name == "Spotify" else 0xA855F7
        )
